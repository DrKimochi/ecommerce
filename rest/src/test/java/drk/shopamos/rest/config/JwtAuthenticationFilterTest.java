...
class JwtAuthenticationFilterTest {
    ...
    void doFilterInternal_whenInvalidToken_thenSecurityContextNotSet() throws Exception {
        ...
        when(jwtTokenHelper.extractUsername(SOME_TOKEN)).thenReturn(SOME_USERNAME);
        when(accountService.loadUserByUsername(SOME_USERNAME)).thenReturn(account);
        when(jwtTokenHelper.isTokenValid(SOME_TOKEN, account)).thenReturn(false);
    ...
    void doFilterInternal_whenValidToken_thenSecurityContextIsSet() throws Exception {
        ...
        when(jwtTokenHelper.extractUsername(SOME_TOKEN)).thenReturn(SOME_USERNAME);
        when(accountService.loadUserByUsername(SOME_USERNAME)).thenReturn(account);
        when(jwtTokenHelper.isTokenValid(SOME_TOKEN, account)).thenReturn(true);
        ...
        assertThat(authToken.getValue().getPrincipal(), is(account));
